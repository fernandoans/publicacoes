...
\codigo{\$ sudo add-apt-repository ppa:otto-kesselgulasch/gimp} \\
\codigo{\$ sudo apt install gimp gimp-gmic gimp-data gimp-data-extras}

OpenJava, Compactação RAR e Bibliotecas Básicas para vídeo: \\
\codigo{\$ sudo apt install openjdk-8-jdk unrar rar libdvd.pkg}

Configurar o Gnome: \\
\codigo{\$ sudo apt install gnome-tweak-tool}

Conky para informações gerenciais: \\
\codigo{\$ sudo apt install conky-all}

Compilador Assembly (NASM): \\
\codigo{\$ sudo apt install nasm}

Dicionário de Inglês: \\
\codigo{\$ sudo apt-get install artha}

Gerenciar melhor as Fontes (tipos de letra) do Sistema: \\
\codigo{\$ sudo apt-get install font-manager}

Criar partituras: \\
\codigo{\$ sudo apt-get install musescore}
...
Após instalar, acessar o dConf para o Nautilus para permitir a entrada do endereço. Caminho: org | gnome | nautilus | preferences. Ativar a opção: \codigo{ always-use-location-entry}
...
\codigo{\$ sudo apt install gcc g++ codeblocks}

Para criação de jogos utilizo a biblioteca SFML para C++ \\
\codigo{\$ sudo apt install libsfml-dev}
...
\codigo{\$ unzip xmind-8-update7-linux.zip -d xmind}

Mover para opt: \\
\codigo{\$ sudo mv xmind /opt/}

Preparar os diretório, permitindo sua leitura: \\
\codigo{\$ sudo chmod a+w /opt/xmind/XMind\_amd64/configuration}

Editar o arquivo de configuração: \\
\codigo{\$ sudo nano /opt/xmind/XMind\_amd64/XMind.ini}

E realizar os seguinte itens: \\
\codigo{ TROCAR na Lin 2: ./configuration} \\
\codigo{ Para: /opt/xmind/XMind\_amd64/configuration}

\codigo{ TROCAR na Lin 4: ../workspace} \\
\codigo{ Para: /home/USERNAME/workspace} \\
\codigo{ ADICIONAR na Lin 15: abaixo do -vmargs} \\
\codigo{ --add-modules=java.se.ee}

Criar um diretório para as Fontes: \\
\codigo{\$ sudo mkdir -p /usr/share/fonts/truetype/xmind}

Copiar as Fontes: \\
\codigo{\$ sudo cp -R /opt/xmind/fonts/* /usr/share/fonts/truetype/xmind/}

Recarregar as Fontes: \\
\codigo{\$ sudo fc-cache -f}

Criar um arquivo para o Desktop: \\
\codigo{\$ sudo nano /usr/share/applications/xmind.desktop}

Adicionar o seguinte conteúdo: \\
\codigo{ [Desktop Entry] \\ 
...
\codigo{\$ sudo apt install sshfs}

Note | Criar uma pasta de montagem: \\
\codigo{\$ sudo mkdir /mnt/r3d3}

Note | Dar permissão a pasta para o usuário local: \\
\codigo{\$ sudo chmod -R 777 /mnt/r3d3}

Serv | Criar uma pasta de ligação: \\
\codigo{\$ mkdir ~/partilhar}

Serv | Instalar o SSH-Server: \\
\codigo{\$ sudo apt install openssh-server}

Note | Realizar a ligação: \\
\codigo{\$ sshfs fernando@XXX.XXX.SS.S:/home/fernando/partilhar /mnt/r3d3}
...
\codigo{\$ sudo apt-add-repository ppa:dartsim/ppa}

Próximo passo é instalar as bibliotecas completas: \\
\codigo{\$ sudo apt install libdart6-all-dev}
...
\codigo{\$ mkdir development}

Acessar a pasta: \\
\codigo{\$ cd development}

Descompactar na pasta: \\
\codigo{\$ tar xf $\sim$/Downloads/flutter\_linux\_vXXXXXX.tar.xz}

Adicionar o flutter na variável PATH: \\
\codigo{\$ export PATH=`pwd`/flutter/bin:\$PATH}

Verificar o estado do Flutter: \\
\codigo{\$ flutter doctor}

Verificar a versão atual do Flutter: \\
\codigo{\$ flutter channel}

Atualizar o Flutter: \\
\codigo{\$ flutter upgrade}
...
\codigo{\$ sudo apt-get install texlive-latex-extra \\
...
\codigo{\$ gsettings set org.gnome.shell.extensions.dash-to-dock click-action \\ 'minimize-or-previews'}

\textbf{Editor Atom} | Instalar os seguintes plugins: \\
\codigo{ PlatformIO IDE Terminal \\
Assembler}
...
\codigo{\$ sudo apt-add-repository ppa:audio-recorder/ppa \\
	sudo apt-get update \\

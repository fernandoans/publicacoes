...
\begin{tikzpicture}
  \node [mybox] (box){%
    \begin{minipage}{0.3\textwidth} \vspace{0.5em}
	  Configurar usuário do GitHub ao Sistema: \\
      \codigo{git config –global user.name \aspas{[nome]}} \\[2mm]
...
\begin{tikzpicture}
  \node [mybox] (box){%
    \begin{minipage}{0.3\textwidth} \vspace{0.5em}
      Verificar mudanças do diretório de trabalho: \\
      \codigo{git status} \\[2mm]
      Comparar alterações entre 2 \textit{branchs}: \\
      \codigo{git diff [branch1] [branch2]} \\[2mm]
      Adicionar arquivo ao próximo \textit{commit}: \\
      \codigo{git add [arquivo]} \\[2mm]
      Realizar \textit{commit} com mensagem: \\
      \codigo{git commit -m \aspas{mensagem}} \\[2mm]
      Marcar o \textit{commit} atual: \\
      \codigo{git tag [nome]} \\[2mm]
      Modificar \textit{branch} atual: \\
      \codigo{git switch -C [próximo] [anterior]} \\[2mm]
      Enviar arquivos adicionados ao repositório remoto: \\
...
\begin{tikzpicture}
  \node [mybox] (box){%
    \begin{minipage}{0.3\textwidth} \vspace{0.5em}
	  Listar configurações do repositório: \\
      \codigo{git remote -v} \\[2mm]
      Mostrar informações do repositório: \\
      \codigo{git remote show [nome]} \\[2mm]
	  Verificar \textit{commits} realizados (\textbf{q} para sair): \\
      \codigo{git log} \\[2mm]
...
\begin{tikzpicture}
  \node [mybox] (box){%
	\begin{minipage}{0.3\textwidth} \vspace{0.5em}
  	  Listar \textit{branches} existentes: \\
	  \codigo{git branch -av} \\[2mm]
	  Modificar cabeçalho da \textit{branch}: \\
	  \codigo{git checkout [branch]} \\[2mm]
	  Criar nova \textit{branch} com base na corrente: \\
	  \codigo{git branch [novaBranch]} \\[2mm]
	  Criar nova trilha: \\
      \codigo{git checkout --track [remote/Branch]} \\[2mm]
	  Remover \textit{brach} local: \\
      \codigo{git branch -d [branch]} \\[2mm]
	  Eliminar \textit{brach} remota: \\
  ...
  \node [mybox] (box){%
	\begin{minipage}{0.3\textwidth} \vspace{0.5em}
 	  \begin{wrapfigure}{l}{0.3\textwidth}
        \vspace{-\baselineskip}
        \includegraphics[width=0.8\linewidth]{imgGit/emDia} 
      \end{wrapfigure}  
...
\begin{tikzpicture}
  \node [mybox] (box){%
	\begin{minipage}{0.3\textwidth} \vspace{0.5em}
      ...
      \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{imgGit/merge}
      \end{figure}
   	  Realizar mesclagem: \\
	  \codigo{git merge [branch]} \\[2mm]
  	  ...
  	  \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{imgGit/rebase}
      \end{figure}
	  Realizar \textit{rebase}: \\
      \codigo{git rebase [branch]} \\[2mm]
	  Interromper \textit{rebase}: \\

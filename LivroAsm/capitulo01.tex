...
Vamos verificar se o valor informado é maior que 4 ou menor que 1, caso seja desviamos o fluxo para uma label chamada \textbf{msterro}, caso contrário podemos prosseguir para obter os valores a serem calculados.
...
Reobtemos o valor digitado em \textbf{opc}, convertemos para inteiro e podemos realizar os comparativos para saber qual opção nosso usuário selecionou. E chegamos no ponto de saída, pois qualquer valor correto desviará o fluxo do programa para outro ponto.
\begin{lstlisting}[]
saida:
...
Mostrar a mensagem de término e encerrar a seção. Mas vai continuar o código? Compilador poderia seguir em frente após "encerrar a seção" (naquele último comando "int SYS\_CALL")? A resposta é não, um vez que recebeu o aviso para encerrar o programa termina, só vai entrar nesses pontos abaixo se "saltar" do marcador \textbf{saída}.
...
\section{Programa 1.7 - Par ou Ímpar}\index{Conceitos Introdutórios}
...
\subsection{Resposta do Problema}\index{Conceitos Introdutórios}
...
O bloco do marcador \textit{saida} encerra nosso programa. Basta compilar, linkeditar e executar para testarmos diversos valores de 3 dígitos, se desejar pode mudar essa característica. E aqui está o fluxograma deste programa para um melhor entendimento:
\begin{figure}[H]
	\centering
